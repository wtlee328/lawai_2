-- Migration: Add multi-field semantic search function
-- This adds the match_cases_multi_field function for enhanced semantic search

-- Multi-field semantic search function for case_2 table
-- Supports searching across content, dispute, opinion, and result embeddings
...
-- Ensure HNSW indexes exist for all embedding fields
CREATE INDEX IF NOT EXISTS case_2_content_embedding_hnsw_idx 
  ON case_2 USING hnsw (content_embedding vector_cosine_ops)
  WITH (m = 16, ef_construction = 64);

CREATE INDEX IF NOT EXISTS case_2_dispute_embedding_hnsw_idx 
  ON case_2 USING hnsw (dispute_embedding vector_cosine_ops)
  WITH (m = 16, ef_construction = 64);

CREATE INDEX IF NOT EXISTS case_2_opinion_embedding_hnsw_idx 
  ON case_2 USING hnsw (opinion_embedding vector_cosine_ops)
  WITH (m = 16, ef_construction = 64);

CREATE INDEX IF NOT EXISTS case_2_result_embedding_hnsw_idx 
  ON case_2 USING hnsw (result_embedding vector_cosine_ops)
  WITH (m = 16, ef_construction = 64);

-- Add comment for documentation
COMMENT ON FUNCTION match_cases_multi_field IS 
'Multi-field semantic search function that supports weighted search across content, dispute, opinion, and result embeddings with configurable field weights and similarity thresholds.';