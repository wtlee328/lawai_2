-- Fix return type issues in match_cases_multi_field function
...
BEGIN
  ...
  IF match_cases_multi_field.content_embedding IS NOT NULL AND match_cases_multi_field.dispute_embedding IS NULL AND match_cases_multi_field.opinion_embedding IS NULL AND match_cases_multi_field.result_embedding IS NULL THEN
    ...
    SELECT 
      c.case_id::text,
      COALESCE(c.content, '')::text as case_content,
      COALESCE(c.summary, '')::text,
      COALESCE(c.topic, '')::text as case_topic,
      COALESCE(c.date::text, '')::text as case_date,
      COALESCE(c.court, '')::text,
      COALESCE(c.dispute, '')::text,
      COALESCE(c.opinion, '')::text,
      COALESCE(c.result, '')::text,
      COALESCE(c.url, '')::text,
      COALESCE(c.laws, '{}'::jsonb)::jsonb,
