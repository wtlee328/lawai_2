-- Fix embedding dimensions in multi-field search function
-- content_embedding: 1536 dimensions
-- dispute_embedding, opinion_embedding, result_embedding: 512 dimensions
...
BEGIN
  ...
  IF query_content_embedding IS NOT NULL AND 
     query_dispute_embedding IS NULL AND 
     query_opinion_embedding IS NULL AND 
     query_result_embedding IS NULL THEN
    ...
    SELECT 
      ...
      c.url,
      c.laws,
      (1 - (c.content_embedding <=> query_content_embedding))::float as content_similarity,
      0.0::float as dispute_similarity,
      0.0::float as opinion_similarity,
      0.0::float as result_similarity,
      (1 - (c.content_embedding <=> query_content_embedding))::float as weighted_similarity
    FROM case_2 c
    WHERE c.content_embedding IS NOT NULL
      AND (1 - (c.content_embedding <=> query_content_embedding)) > match_threshold
    ORDER BY c.content_embedding <=> query_content_embedding
    LIMIT match_count;
    
  ELSE
    -- For multi-field, use a simpler approach with reasonable limits
    RETURN QUERY
    SELECT 
      c.case_id,
      c.content as case_content,
    ...
    WHERE 
      ...
      (query_result_embedding IS NULL OR c.result_embedding IS NOT NULL) AND
